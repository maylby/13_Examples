...
// Первым делом, отметить, ясна ли задача? 
// В частности:

// * Что значит «Дан текст»? 
// Ввёл пользователь? 
// Считали из файла? 
// Взяли с базы данных, из какого-то сервиса может быть, скачали и так далее. 
// Поэтому чётко определяем для себя, что значит, дан. 
// Мы будем считать, что он просто будет храниться, как отдельная строка.

// * Что значит «чёрточками»? 
// тире, дефис и ещё можно добавить минус.

// * Какой алфавит? 
// Кириллица, в этом случае буква «к» одна.
// Латиница, тогда буква «к» уже какой-то другой символом. 
// При этом буквы «C» и «с» мало того, что на одной кнопке находиятся, так ещё и выглядят одинаково. И
// Но для компьютера это абсолютно разные символы.

// Перейдём к написанию кода. 
// Рекомендую добавлять условия задач, которые решаете, 
// в качестве комментария, чтобы можно было понять, что делать.
