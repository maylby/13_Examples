...
string text = "(1,2) (2,3) (3,4) (4,5)"; // строка, содержащая 4 точки
                .Replice("(", "") // заменяем отрывающую скобку на пробел
                .Replice(")", "") // заменяем закрывающую скобку на пробел
                ;
...
// тип данных "var" позволяет работать с заранее неизвестным типом переменных
                ...
                .Select(e => (int.Parse(e[0], int.Parse(e[1])))) // заменяет внутренний цикл "k"
                // из массива массивов формируем массив кортежей
                .ToArray(); // результат оформляем, как массив

for (int i = 0; i < data.Length; i++)
{
    Console.WriteLine(data[i].Item1*10); // получаем 4 строки, содержащих координату каждой точки
    // добавление в запись "Item1*10" увеличивает вторую координату в 10 раз
